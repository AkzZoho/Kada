...
function buildWhatsAppText(bill: Bill, shopName: string): string {
  const line = '─────────────────────';
  const items = bill.items.map((i) => `  • ${i.name} ×${i.quantity} ${i.unit} = ${fmt(i.lineTotal)}`).join('\n');
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal bill-view" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-footer">
          ...
          <button className="btn" style={{ background: '#25D366', color: '#fff', display: 'flex', alignItems: 'center', gap: 6 }} onClick={shareWhatsApp}>
